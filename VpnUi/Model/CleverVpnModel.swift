...
public class CleverVpnModel: ObservableObject {
    ...
    func setLocation(selectedLocation: Location?) {
        Task {
            if let key = userInfo?.key {
                userInfo = UserInfo(key: key, locationId: selectedLocation?.id)
                await VpnClient.shared.updateUserInfo(userInfo: userInfo!)
            }
    ...
    func setProtocolType(protocolType:  ProtocolType) {
        Task {
            if let key = userInfo?.key {
                userInfo = UserInfo(key: key, locationId: userInfo?.locationId ,protocolType: protocolType)
                await VpnClient.shared.updateUserInfo(userInfo: userInfo!)
            }
        }
    }
...
//    func getAppId() {
//        Task {
//            let _appUUID =  await VpnClient.shared.getAppId()
//            await MainActor.run {
//                appUUID = _appUUID
//            }
//        }
