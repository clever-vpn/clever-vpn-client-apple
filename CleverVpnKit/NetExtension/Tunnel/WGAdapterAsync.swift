...
class WGAdapterAsync {
    ...
    func update(tunnelConfiguration: TunnelConfiguration) async -> Result<Void, WireGuardAdapterError> {
        return await withCheckedContinuation { continuation in
            self.wireguardAdapter.update(tunnelConfiguration: tunnelConfiguration) { adapterError in
                if let adapterError = adapterError {
                    wg_log(.error, message: "Failed to update WireGuard adapter: \(adapterError.localizedDescription)")
                    continuation.resume(returning: .failure(adapterError))
                } else {
                    continuation.resume(returning: .success(()))
                }
...
extension WireGuardAdapterError: @retroactive CustomStringConvertible {
