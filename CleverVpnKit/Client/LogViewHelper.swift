...
class LogViewHelper {
    ...
    func fetchLogEntriesSinceLastFetch(completion: @escaping ([LogEntry]) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let newCursor = view_lines_from_cursor(self.log, self.cursor, &logEntries) { cStr, timestamp, ctx in
                ...
                let dateString = ISO8601DateFormatter.string(
                    from: date, timeZone: TimeZone.current, formatOptions: LogViewHelper.formatOptions)
    ...
    func asyncFetchLogEntriesSinceLastFetch() async -> [LogEntry] {
        return await withCheckedContinuation { continuation in
            fetchLogEntriesSinceLastFetch { entries in
